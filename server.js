...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const childPost = {
      ...
      '/sos': (db, d, b) => { const msg = b.message || b.note || 'SOS from child'; const lat = b.latitude || b.lat || 0; const lon = b.longitude || b.lon || 0; db.sos.push({ id: rid(), deviceId: d.deviceId, message: msg, lat: lat, lon: lon, ack: 0, createdAt: now() }); db.alerts.push({ id: rid(), deviceId: d.deviceId, parentId: d.parentId, type: 'SOS', title: 'SOS ALERT', message: msg + (lat ? (' @ ' + lat + ',' + lon) : ''), createdAt: now(), read: false }); },
